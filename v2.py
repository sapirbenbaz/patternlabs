...
class GeminiModelInstance:
    ...
    def create_gemini_instance(self, api_key, temprature):
...
def generate_prompt(question, answer1, answer2):
    return f"""I will give you a question or sentence to complete and two possible answers.
               ...
               You must make sure your answer is correct.
               The question or sentence is: {question}, the first answer is: {answer1}, the second answer is: {answer2}."""


# This method receives the two instances, the prompt and the correct answer. It invokes the two
# instances, in case of a mismatch between the answers returned, we keep creating new instances
# until its response meets the previous instance's response. Once matched, we check wether the
# response is equal to the actual correct answer
def evaluate_models(base_instance, reviewer_instance, prompt, correct_answer):
    base_answer = base_instance.get_model_answer(prompt)
    answers_match = False

    while not answers_match:
        reviewer_answer = reviewer_instance.get_model_answer(prompt)
        answers_match = base_answer == reviewer_answer
        if not answers_match:
            reviewer_instance = GeminiModelInstance(
                reviewer_instance.instance.google_api_key, round(random.random(), 1)
            )
            base_answer = reviewer_answer
            print("Answers do not match. Creating a new Gemini instance")

    return (base_answer == "A" and correct_answer == 0) or (
        base_answer == "B" and correct_answer == 1
    )
...
def main():
    ...
    base_instance = GeminiModelInstance(api_key, round(random.random(), 1))
    reviewer_instance = GeminiModelInstance(api_key, round(random.random(), 1))
    ...
    try:
        for _ in range(50):
            ...
            # Evaluating the two models' answers and verifying their accuracy
            if evaluate_models(
                base_instance, reviewer_instance, prompt, correct_answer
            ):
                correct_answers += 1
